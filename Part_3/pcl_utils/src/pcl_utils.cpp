...
bool PclUtils::find_cylinder_fit(double x_min, double x_max, double y_min, double y_max, double z_min, double z_max, double dz_tol, double surface_height,
      double& radius, Eigen::Vector3f  &centroid){ 
    vector<int> indices;
    bool ans_valid = true;
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_filtered (new pcl::PointCloud<pcl::PointXYZ>);
    pcl::PassThrough<pcl::PointXYZ> pass; //create a pass-through object    
    ...
    fit_points_to_plane(cloud_filtered, plane_normal, plane_dist);    
    centroid = centroid_;
    double z_mid = (centroid(2) + surface_height) * 0.5;
    pass.setInputCloud(pclTransformed_ptr_); //set the cloud we want to operate on--pass via a pointer
    pass.setFilterFieldName("x"); // we will "filter" based on points that lie within some range of z-value
    pass.setFilterLimits(x_min, x_max);
    pass.filter(*cloud_filtered); //fill this cloud with result
    ...
    pass.setInputCloud(cloud_filtered);
    pass.setFilterFieldName("z"); // we will "filter" based on points that lie within some range of z-value
    pass.setFilterLimits(z_mid-0.02, z_mid+0.02);
    pass.filter(*cloud_filtered);
    int size_ = cloud_filtered->points.size();
    int num_ = 20;
    int it_ = 30;
    double alpha = 0.5;
    double X_ = centroid(0);
    double Y_ = centroid(1);
    double R_ = std::sqrt((cloud_filtered->points[0].x - X_)*(cloud_filtered->points[0].x - X_) + (cloud_filtered->points[0].y - Y_)*(cloud_filtered->points[0].y - Y_));
    for(int i = 0; i < it_; i++)
    {
    		double E_X = .0;
    		double E_Y = .0;
    		...
    		{
    				double x_ = cloud_filtered->points[int(size_ * j / num_)].x;
    				double y_ = cloud_filtered->points[int(size_ * j / num_)].y;
    				double z_ = cloud_filtered->points[int(size_ * j / num_)].z;
    				double e_ = x_*x_ + y_*y_ + X_*X_ + Y_*Y_ - 2*x_*X_ - 2*y_*Y_ - R_*R_;
    				E_X += 4*(X_-x_)*e_;
    				E_Y += 4*(Y_-y_)*e_;
    				E_R -= 4*R_*e_;
    		}
