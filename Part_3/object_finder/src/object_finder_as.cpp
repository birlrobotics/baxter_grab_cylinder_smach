...
bool ObjectFinder::find_upright_coke_can(float surface_height, geometry_msgs::PoseStamped &object_pose) {
    ...
    found_object = pclUtils_.find_cylinder_fit(0.2, 1, -0.5, 0.5, surface_height + 0.13, surface_height + 0.17, 0.001, surface_height, radius, centroid);
    ...
    //
    if (plane_normal(2) < 0) plane_normal = -plane_normal; //in world frame, normal must point UP
    Eigen::Matrix3f R;
    ...
    switch (object_id) {
        case ObjectIdCodes::COKE_CAN_UPRIGHT:
            ...
            if (found_object) {
                ...
                ROS_WARN("could not find upright Coke can");
